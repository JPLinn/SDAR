...
class Net(nn.Module):
    def __init__(self, params):
        ...
        self.mg_pre_w = nn.Linear(
            params.lstm_hidden_dim * params.lstm_layers, params.num_kernels)
        self.mg_pre_mu = nn.Linear(
            params.lstm_hidden_dim * params.lstm_layers, params.num_kernels)
        self.mg_pre_sigma = nn.Linear(
            params.lstm_hidden_dim * params.lstm_layers, params.num_kernels)
        self.mg_w = nn.Softmax(dim=1)
        self.mg_sigma = nn.Softplus()
        self.mg_mu = nn.Sigmoid()
    ...
    def forward(self, x, idx, hidden, cell):
        ...
        pre_w = self.mg_pre_w(hidden_permute)
        mg_w = self.mg_w(pre_w)  # Softmax to make sure Σw equals to 1
        pre_sigma = self.mg_pre_sigma(hidden_permute)
        mg_sigma = self.mg_sigma(pre_sigma)
        # softplus to make sure the intercept is positive
        pre_mu = self.mg_pre_mu(hidden_permute)
        mg_mu = self.mg_mu(pre_mu)

        return torch.squeeze(mg_w), torch.squeeze(mg_mu), \
               torch.squeeze(mg_sigma), hidden, cell
...
def loss_fn(w: Variable, mu: Variable, sigma: Variable, labels: Variable):
    zero_index = (labels != 0)
    normal = torch.distributions.normal.Normal(torch.tensor(0., device=w.device),
                                               torch.tensor(1.0, device=w.device))
    auto_mu = torch.unsqueeze(labels, 1) - mu
    auto_term = w * (2*torch.sqrt(sigma)*torch.exp(normal.log_prob(auto_mu/torch.sqrt(sigma))) +
                     auto_mu*(2*normal.cdf(auto_mu/torch.sqrt(sigma)) - 1))

    cross_mu = torch.unsqueeze(mu, 2).expand(-1, -1, mu.shape[1]) - \
               torch.unsqueeze(mu, 2).expand(-1, -1, mu.shape[1]).permute(0, 2, 1)
    cross_sigma = torch.unsqueeze(sigma, 2).expand(-1, -1, mu.shape[1]) + \
               torch.unsqueeze(sigma, 2).expand(-1, -1, mu.shape[1]).permute(0, 2, 1)
    cross_w = torch.unsqueeze(w, 2).expand(-1, -1, mu.shape[1]) * \
               torch.unsqueeze(w, 2).expand(-1, -1, mu.shape[1]).permute(0, 2, 1)
    cross_term = cross_w * (2*torch.sqrt(cross_sigma)*torch.exp(normal.log_prob(cross_mu/torch.sqrt(cross_sigma))) +
                            cross_mu*(2*normal.cdf(cross_mu/torch.sqrt(cross_sigma)) - 1))
    crps = (torch.sum(auto_term) - torch.sum(cross_term)/2.)/labels.numel()
    return crps
